...
int32_t bd_alloc(int32_t order);
int32_t bd_free(int32_t offset);
int32_t bd_display();
int32_t bd_get_size(int32_t offset);
int32_t bd_get_start(int32_t offset);
    ...
    for (i = 0; i < num_node - 1; i++)
    {
        ...
        else
        {
            if (get_size(node_order) < 10)
            {
                ...
                print_int(get_size(node_order));
            }
            else
            {
                for (j = 0; i < curr_size - 2; i++)
                {
                    ece391_fdputs(1, (uint8_t *)" ");
                }
                print_int(get_size(node_order));
